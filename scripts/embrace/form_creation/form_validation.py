...
class FormValidations(form_creation.FormCreation):
    ...
    def finding_group_element(self, group):
        try:
            ui_group = self.driver.find_elements_by_xpath('//u[@class="ng-binding"]')
            ui_group_data = []
            for i in ui_group:
                ui_group_data.append(i.text)
            print(ui_group_data)

            for j in ui_group_data:
                if j == group:
                    self.group = True
                    break
        except Exception as error:
    ...
    def group_validation(self):
        try:
            self.finding_group_element(self.group_one)
            if self.group:
                self.ui_group_one_name = 'Pass'
                print("Element is a group one")

            self.finding_group_element(self.group_two)
            if self.group:
                self.ui_group_two_name = 'Pass'
                print("Element is a group two")
