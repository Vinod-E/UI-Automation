...
class AdvanceSearch(common_login.CommonLogin):
    ...
    def vendor_job_name_search(self, name, where_search_is_happening):
        try:

            self.driver.find_element_by_name(page_elements.advance_search['vendor_job_name']).clear()
            self.web_element_send_keys_name(page_elements.advance_search['vendor_job_name'], name)
