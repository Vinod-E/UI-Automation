...
class NewInterviewOutputFile(styles.FontColor, submit_feedback_int2.SubmitFeedbackInt2):
    ...
    def __init__(self):
        ...
        index = 0
        excelheaders = ['Settings-Admin', 'Status', 'Schedule-Admin', 'Status', 'SaveDraft-Int1', 'Status',
                        'SubmitFeedback-Int1', 'Status', 'SubmitFeedback-Int2', 'Status']
    ...
    def settings_on_output_report(self):
        try:
            # ------------------------------ Writing Output Data -------------------------------------------------------
            self.ws.write(2, self.settings_sc_col, 'Event Tab', self.style8)
            self.ws.write(3, self.settings_sc_col, 'Advance search', self.style8)
            self.ws.write(4, self.settings_sc_col, 'Event details', self.style8)
            self.ws.write(5, self.settings_sc_col, 'Event Validation', self.style8)
            self.ws.write(6, self.settings_sc_col, 'Floating actions', self.style8)
            self.ws.write(7, self.settings_sc_col, 'Live Interview action', self.style8)
            self.ws.write(8, self.settings_sc_col, 'Live interview validation', self.style8)
            self.ws.write(9, self.settings_sc_col, 'Live schedule', self.style8)
